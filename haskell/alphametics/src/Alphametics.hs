...
data Term a where
  ...
  Mult :: Term Integer -> Term Integer -> Term Integer
  Power :: Term Integer -> Term Integer -> Term Integer
...
maxLength (Mult   l r) = max (maxLength l) (maxLength r)
maxLength (Power  l r) = max (maxLength l) (maxLength r)
...
simpleIntegerTerm = word <|> number

integerOperator :: Parser (Term Integer -> Term Integer -> Term Integer)
integerOperator = try plus <|> try mult <|> power

word :: Parser (Term Integer)
word = Word <$> many1 upper

number :: Parser (Term Integer)
number = Number . read <$> many1 digit

plus :: Parser (Term Integer -> Term Integer -> Term Integer)
plus = trimmed (char '+') *> pure Plus

mult :: Parser (Term Integer -> Term Integer -> Term Integer)
mult = trimmed (char '*') *> pure Mult

power :: Parser (Term Integer -> Term Integer -> Term Integer)
power = trimmed (char '^') *> pure Power
