...
class SeriesTest extends FlatSpec with Matchers {
  ...
  it should "slice" in {
    Series.slices(2, "98273463") should equal(Some(List(List(9, 8), List(8, 2),
      List(2, 7), List(7, 3), List(3, 4), List(4, 6), List(6, 3))))
    Series.slices(3, "982347") should equal(Some(List(List(9, 8, 2), List(8, 2, 3),
      List(2, 3, 4), List(3, 4, 7))))
  }

  it should "slice boundary conditions" in {
    Series.slices(4, "982") should equal(None)
    Series.slices(3, "982") should equal(Some(List(List(9, 8, 2))))
  }
